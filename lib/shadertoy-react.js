...
var Texture_Texture = function Texture(_gl) {
  ...
  _defineProperty(this, "load", function (textureArgs, channelId) {
    ...
    if (!url) {
      return Promise.reject(new Error(SRLOG(_templateObject())));
    }
    ...
    if (isImage === null && isVideo === null) {
      return Promise.reject(new Error(SRLOG(_templateObject2()), url));
    }
...
function src_templateObject2() {
  var data = src_taggedTemplateLiteral(["wrong precision type ", ", please make sure to pass one of a valid precision lowp, mediump, highp, by default you shader precision will be set to highp."]);
...
function (_Component) {
  ...
  function ShadertoyReact(props) {
    ...
    src_defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "preProcessShaders", function (fs, vs) {
      ...
      if (!isValidPrecision) console.warn(SRLOG(src_templateObject2(), precision));
      var fsString = precisionString.concat(fs).replace(/texture\(/g, 'texture2D(');
    ...
    src_defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "setUniforms", function (timestamp) {
      ...
      if (_this.uniforms.iFrame.isNeeded) {
        ...
        gl.uniform1f(_timeDeltaUniform, _this.uniforms.iFrame.value++);
      }
